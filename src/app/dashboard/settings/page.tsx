...
type SettingsSection = 'general' | 'financials' | 'team' | 'security';

export default function SettingsPage() {
  const [activeSection, setActiveSection] = useState<SettingsSection>('financials'); // Set financials as default for testing/visibility
  ...
  const discardChanges = () => {
    setHasChanges(false);
    // In real app, re-fetch from Firebase
  };
      ...
      <div className="flex flex-col md:flex-row gap-12 mt-6">
        ...
        <main className="flex-1 space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            <div className="space-y-6">
              <Card className="p-0 overflow-hidden shadow-sm">
                ...
                <div className="p-6 space-y-4">
                  <TeamMember 
                    name="Marie-Pier Denis" 
                    email="admin@massagenow.com" 
                    role={t('settings.team.role_super')} 
                    isCurrent 
                  />
                  <TeamMember 
                    name="Yvan Belibi" 
                    email="yvan@pro-agent.com" 
                    role={t('settings.team.role_support')} 
                  />
                </div>
              </Card>
            </div>
          ...
          {activeSection === 'security' && (
            <div className="space-y-6 text-center py-20 bg-white border border-dashed border-lightSage rounded-2xl">
               <ShieldCheck className="mx-auto text-mediumSage/20" size={64} />
               <p className="text-mediumSage font-abeezee italic">Les journaux d'audit et les paramètres de sécurité avancés seront disponibles dans la v1.1</p>
            </div>
          )}
          ...
          <div className="bg-charcoal text-white rounded-2xl px-8 py-5 shadow-2xl flex items-center gap-10 border border-white/10 backdrop-blur-lg">
            ...
            <div className="flex items-center gap-4">
              <button 
                ...
                className="text-sm font-bold text-brokenWhite/60 hover:text-white transition-colors"
              >
                {t('settings.discard')}
              </button>
              <Button 
...
const NavButton = ({ active, onClick, icon: Icon, label }: { active: boolean, onClick: () => void, icon: any, label: string }) => (
  <button 
    onClick={onClick}
    className={cn(
      "w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-bold text-sm font-abeezee",
      active 
